...
const useAdd_product = () => {
  ...
  const imageUploadRequestHandler = () => {
    ...
    const multipleVideoPreviewUploadRequest = () => {
      let arr = [];
      for (const file of videoPreview) {
        if (file !== null) {
          arr.push(file);
        }
      }
        ...
        axios.all(multiVideoUploadRequest()).then((res: any) => {
          ...
          for (let x = 0; x < videoFiles.length; x++) {
            for (let y = 0; y < videoFiles[x].length; y++) {
              ...
              productInfo.options[x].files.push({
                type: "VIDEO",
                imageUrl: resPreview[index]?.data.result.resourceUrl,
                resourceUrl: result,
                width: resPreview[index]?.data.result.width,
                height: resPreview[index]?.data.result.height,
              });
              index += 1;
            }
          }
  ...
  //   setProductInfo({ ...productInfo, weight: e.target.value });
  // };
  const [videoPreview, setVideoPreview] = useState(new Array(10).fill(null));

  /** 동영상 미리보기 이미지 얻어서 파일객체로 저장 */
  const getPreview = (e: any, index: number) => {
