...
import {
  ...
  productDisplayOffRequest,
  productDisplayOnRequest,
} from "../../../utils/api";
...
const useId = () => {
  ...
  const productDetailRequestHandler = () => {
    ...
    if (at && atExpire) {
      productDetailRequest("at", window.location.pathname.split("/")[3]).then(
        async (res) => {
          console.log(res);
  ...
  const saleHandler = () => {
    ...
    if (info.display === false) {
      ...
      productDisplayOnRequest(at, info.productNo);
      return;
    }
    ...
    let at = localStorage.getItem("at");
    let rt = localStorage.getItem("rt");
    productDisplayOffRequest(at, info.productNo);
    return;
  };
      ...
      <Container>
        <ProductInfoContainer>
          ...
          <MobileSaleContainer>
            ...
            <SlideButton
              sale={info && info.display}
              onClick={() => saleHandler()}
            >
              <SlideButtonDot sale={info && info.display} />
            </SlideButton>
          </MobileSaleContainer>
          {/** 모바일에서의 타이틀 */}
...
const Title = styled.div`
  ...
  color: #121822;
`;
const MobileSaleContainer = styled.div`
  display: none;
  align-items: center;
...
`;
const SlideButton = styled.div<{ sale: boolean }>`
  display: flex;
  width: 52px;
  height: 28px;
  align-items: center;
  ...
  background-color: ${(props) => {
    return props.sale ? `#E1FF20` : `#DEE8EC`;
  }};
  border: ${(props) => {
    return props.sale ? `1px solid #D6F221` : `1px solid #DEE8EC`;
  }};
  border-radius: 14px;
  ...
  cursor: pointer;
`;
const SlideButtonDot = styled.div<{ sale: boolean }>`
  width: 16px;
  height: 16px;
  background-color: ${(props) => {
    return props.sale ? `#121822` : `#A4B0B3`;
  }};
  border-radius: 100%;
  transition: all 0.1s;

  transform: ${(props) => {
    return props.sale ? `translateX(19.1px)` : ``;
  }};
`;
