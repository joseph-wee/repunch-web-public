...
const PopUpSelectDesign = ({
  ...
  useEffect(() => {
    ...
    sessionStorage.getItem("desgins")
      ? setTempList([
          ...JSON.parse(desgins || "").map((el: any) => {
            return { ...el, isChecked: false, value: 0 };
          }),
        ])
      : designsRequest().then((res: any) => {
          sessionStorage.setItem("desgins", JSON.stringify(res?.data.result));
          setTempList([
...
const Container = styled.div<{ selectCategory: string }>`
  z-index: 3;
  display: ${(props) => {
    return props.selectCategory === "design" ? "flex" : "none";
  }};
  align-items: center;
...
export default PopUpSelectDesign;
